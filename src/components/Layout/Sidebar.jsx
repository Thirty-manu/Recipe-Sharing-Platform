import { Home, TrendingUp, Star, BookOpen, PlusCircle, User, LogOut, X, Shield } from "lucide-react";
...
export default function Sidebar({ page, setPage, user, onLogout, mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      {mobileOpen && (
        <div onClick={() => setMobileOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 40, display: "none" }} className="mobile-overlay" />
      )}
      <aside style={{ width: 240, background: "var(--bg-sidebar)", height: "100vh", display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)", flexShrink: 0, position: "relative", zIndex: 50, transition: "transform .25s ease" }} className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
        ...
        {admin && (
          <div style={{ margin: "10px 8px 0", background: "rgba(88,101,242,.12)", border: "1px solid rgba(88,101,242,.3)", borderRadius: 8, padding: "7px 12px", display: "flex", alignItems: "center", gap: 7 }}>
          ...
          {NAV.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button key={id} onClick={() => { setPage(id); setMobileOpen(false); }}
                style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 8, marginBottom: 2, background: active ? "rgba(88,101,242,.18)" : "none", color: active ? "var(--accent)" : "var(--text-muted)", fontWeight: active ? 600 : 400, fontSize: 14, transition: "all .15s" }}
        ...
        <div style={{ padding: "12px 8px", borderTop: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 8 }}>
            <div style={{ position: "relative" }}>
              ...
              {admin && (
                <div style={{ position: "absolute", bottom: -2, right: -2, width: 14, height: 14, background: "var(--accent)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid var(--bg-sidebar)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ fontSize: 11, color: admin ? "var(--accent)" : "var(--text-muted)" }}>{admin ? "Super Admin" : "Chef"}</p>
            </div>
