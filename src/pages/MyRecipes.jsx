...
export default function MyRecipes({ user, userProfile, search, onOpen }) {
  ...
  const filtered = recipes.filter(r => { const q = search.toLowerCase(); return !q || r.title?.toLowerCase().includes(q); });
  const handleDelete = async (id) => { if (!confirm("Delete this recipe?")) return; try { await deleteRecipe(id); toast.success("Recipe deleted"); } catch { toast.error("Couldn't delete"); } };
  ...
  return (
    <div style={{ padding: "24px 28px", height: "100%", overflowY: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <button onClick={() => setAdding(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "var(--accent)", color: "#fff", borderRadius: 8, fontSize: 14, fontWeight: 600 }}><PlusCircle size={15} /> New Recipe</button>
      </div>
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", paddingTop: 80, color: "var(--text-muted)" }}><div style={{ fontSize: 48, marginBottom: 12 }}>📖</div><p style={{ fontSize: 16, fontWeight: 600 }}>No recipes yet</p><p style={{ fontSize: 14, marginTop: 6 }}>Start sharing your culinary creations!</p></div>
      ) : (
